...
public class GetCmRequestValueValidate {
    public static void getCmRequestValueValidate(HttpServletRequest request) {
        //union_name校验
        String union_name = request.getParameter("union_name");
        PmUnionnameValidate.pmunionnamevalidate(union_name);

        //downx,dowy校验
        String downx = request.getParameter("downx");
        String downy = request.getParameter("downy");
        if (!"头条".equals(union_name)) {
            CmDownxValidate.cmdowxvalidate(downx);
            CmDownyValidate.cmdownyvalidate(downy);
        }
        //ad_type校验
