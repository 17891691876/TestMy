...
import bigtest.validate.request.pm.*;
...
@Slf4j
@Component  //声明一个组件
public class GetRmRequestValueValidate {
    public static void getRmRequestValueValidate(HttpServletRequest request){
        //union_name校验
        ...
        RmIsSuccessValueValidate.rmissuccessvalidate(is_success);

        //reason校验
        String reason=request.getParameter("reason");
        RmReasonValueValidate.rmreasonvalidate(reason);

        //delay校验
        String delay=request.getParameter("delay");
        RmDelayValidate.rmdelayvalidate(delay);
        ...
        if (!"true".equals(template) && !"3".equals(type)){
        ...
        PmEntranceValidate.pmentrancevalidate(entrance);
